...
func (f *TextFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, timestampFormat string, colorScheme *compiledColorScheme) {
	...
	if entry.HasCaller() {
		caller = fmt.Sprintf("%s:%d",
			formatFilePath(entry.Caller.File), entry.Caller.Line)
	}
	...
	if f.DisableTimestamp {
		_, _ = fmt.Fprintf(b, "%s%s%s "+messageFormat, level, caller, prefix, message)
	} else {
		...
		if caller != "" {
			_, _ = fmt.Fprintf(b, "%s [%s] %s%s "+messageFormat, colorScheme.TimestampColor(timestamp), caller, level, prefix, message)
		} else {
